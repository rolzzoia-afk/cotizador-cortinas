 ...
 * que `precioMlPorCod` del motor: manda el código declarado, y sin código
 * declarado (o con uno roto) manda la más cara del catálogo. El declarado es un
 * PISO: una tela más cara lo levanta, pero SOLO cuando esa tela va en la
 * cotización, así que acá se muestra como aviso y no como el precio. Se calcula
 * acá y no se llama al motor porque esta tabla trabaja sobre el BORRADOR, que
 * todavía no es un `ReglasPrecios` completo.
 */
function precioEfectivo(fam: string, codInt: string, catalogo: CatalogoProductos) {
  ...
  if (pDeclarado > 0) {
    return {
      ...
      manda: declarado,
      roto: '',
      masCara: max.precio > pDeclarado ? max : null,
    };
  }
  return { precio: max.precio, manda: max.codInt, roto: declarado, masCara: null };
}
        ...
        <table className="w-full text-xs">
          ...
          <tbody>
            {claves.map((fam) => {
              ...
              const { precio, manda, roto, masCara } = precioEfectivo(fam, codInt, catalogo);
              return (
                <tr key={fam} className="border-t">
                  ...
                  <td className="px-2 py-1">
                    ...
                    {roto ? (
                      ...
                      // El código escrito es un PISO: una tela más cara lo
                      // levanta, pero solo en las cotizaciones que la lleven.
                      // Decirlo acá evita la sorpresa de ver otro precio.
                      <div className="mt-0.5 text-[0.65rem] text-muted-foreground">
                        {masCara.codInt} vale más ({formatCLP(Math.round(masCara.precio))}/m): manda
                        solo en las cotizaciones que la lleven
                      </div>
                    ) : !codInt.trim() ? (
                      <div className="mt-0.5 text-[0.65rem] text-muted-foreground">
  ...
  return (
    <section className="rounded-lg border bg-card p-5">
      ...
      <p className="mb-3 text-xs text-muted-foreground">
        ...
        <strong>Con una excepción:</strong> si la cotización lleva una tela de la familia que vale
        MÁS que este código, se cobra esa. Una tela cara que nadie compró no le sube el precio a la
        familia.
      </p>
      <p className="mb-3 text-xs text-muted-foreground">
